#include "sql_command_class.h"

#using namespace std;

DataBase::DataBase(const string connection) : connection_{ connection } { cout << "Connected to database successfully!" << endl; }
...
void DataBase::createDatabaseTables()
{
	try
	{
		...
		txn.commit();
		cout << "A database structure (tables) created" << endl;
	}
	catch (const exception& ex)
	{
		SetConsoleCP(CP_UTF8);
		SetConsoleOutputCP(CP_UTF8);
		cerr << "Exception happened: " << ex.what() << endl;
	}

}

void DataBase::addClient(const string& name, const string& surname, const string& email, const string& phone) 
{
	try
	{
		pqxx::work txn(connection_);
		connection_.set_client_encoding("UTF8");
		pqxx::result res = txn.exec_params("INSERT INTO clients (name, surname, email) VALUES ($1, $2, $3) REURNING id", name, surname, email);
		if (!res.empty()) {
			int client_id = res[0][0].as<int>();
			txn.exec_params("INSERT INTO phones (client_id, phone) VALUES ($1, $2)", client_id, phone);
			txn.commit();
			cout << "Client added with ID: " << client_id << endl;
		}
		else {
			txn.abort();
			cerr << "Failed to add client" << endl;
		}
